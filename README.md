...
```powershell
# 1. 生成 keystore（keytool 一次性签名）
keytool -genkey -v -keystore android/app/release.keystore `
  -alias duoduo-deep -keyalg RSA -keysize 2048 -validity 10000 `
  -dname "CN=<你的名字>, O=<组织>, L=<城市>, ST=<省>, C=CN" `
  -storepass <你的密码> -keypass <你的密码>

# 2. 填 .env.local（gitignored，密码留这里）
Copy-Item .env.local.example .env.local
# 编辑 .env.local 填入 KEYSTORE_PASSWORD / KEY_PASSWORD

# 3. 一键脚本：写 android/key.properties + 设 4 个 GitHub secret
pwsh scripts/setup-release-signing.ps1
